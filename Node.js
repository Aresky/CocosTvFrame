 ...
 */
st.Utils.Node.attachNodes = function(parentNode, childNode, posDesc, cfgObj) {
    ...
    if (posDesc) {
        var _pos = st.Utils.Node.getPosByDesc(parentNode, posDesc);
...
//更新节点状态
st.Utils.Node.updateNodeState = function (childNode, posDesc, cfgObj) {
...
st.Utils.Node.getPosByDesc = function(parentNode, posDesc) {
    var _pos = cc.p(0, 0);
...
st.Utils.Node.getPointOfScreen = function(desc) {
...
st.Utils.Node.getPointOfNodeCenter = function(node) {
...
st.Utils.Node.getPointOfNodeCenterLeft = function(node) {
...
st.Utils.Node.getPointOfNodeCenterTop = function(node) {
...
st.Utils.Node.getPointOfNodeCenterRight = function(node) {
...
st.Utils.Node.getPointOfNodeCenterBot = function(node) {
...
st.Utils.Node.getPointOfNodeLeftTop = function(node) {
...
st.Utils.Node.getPointOfNodeLeftBot = function(node) {
...
st.Utils.Node.getPointOfNodeRightTop = function(node) {
...
st.Utils.Node.getPointOfNodeRightBot = function(node) {
...
//描绘Node边框，方便调试
st.Utils.Node.makeDebugBoundRect = function(node, c4f0) {
...
//描绘Node锚点，方便调试
st.Utils.Node.makeDebugPoint = function(node, c4f0, c4f1) {
...
st.Utils.Node.makeDebugBound = function(node) {
    st.Utils.Node.makeDebugBoundRect(node);
    st.Utils.Node.makeDebugPoint(node);
};

st.Utils.Node.replaceNode = function(ccbNode, nodeTag, newNode,noReplaceAnchor,noScale){
...
st.Utils.Node.convertPoint = function(targetNode, oriNode, pos) {
...
st.Utils.Node.setStereoFocus = function(node,type, duration, scaleValue) {
...
//取消立体焦点效果
st.Utils.Node.unStereoFocus = function(node, duration) {
    if(!node.stereoFocused){
        return;
    }
...
st.Utils.Node.setFocused = function(node, actiontType){
    if(node.stereoFocused){
        return;
    }
    node.stereoFocused = true;
...
st.Utils.Node.setUnFocused = function(node){
    if(!node.stereoFocused){
        return;
    }
    node.stereoFocused = false;
...
st.Utils.Node.clearFocusInfo = function(node){
    if(!node.stereoFocused){
        return;
    }
    node.stereoFocused = false;
...
// st.Utils.Node.flipFocusBoard = function(node, duration){
//     var board_tag = 1000;
//     var board = node.getChildByTag(board_tag);
...
//@lbPoint  截图起始点左下角坐标
//@blurSize 截图尺寸
st.Utils.Node.screenShot = function(lbPoint, blurSize){
...
st.Utils.Node.blurSpriteWithPath = function(_sprite){
    //获取垂直模糊shader
...
//sprite倒影特效处理
st.Utils.Node.invertedSprite = function(_sprite){
